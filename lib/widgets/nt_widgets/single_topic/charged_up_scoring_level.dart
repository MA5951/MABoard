...
class ChargedUpScoringLevelModel extends SingleTopicNTWidgetModel {
  ...
  ChargedUpScoringLevelModel.fromJson({
    ...
    required Map<String, dynamic> jsonData,
            ...
            tryCast(jsonData['grid_asset']) ?? 'assets/chargeup/grid.png',
        imageAspectRatio =
        ...
        borderWidth =
            tryCast<num>(jsonData['border_width'])?.toDouble() ?? 3.0,
        borderRadius =
            tryCast<num>(jsonData['border_radius'])?.toDouble() ?? 8.0,
        showLabels = tryCast(jsonData['show_labels']) ?? true,
        labelOnlyWhenSelected =
            tryCast(jsonData['label_only_selected']) ?? true,
  ...
  Widget _doubleField(String label, double value, void Function(double) setValue) {
  ...
  List<Widget> getEditProperties(BuildContext context) => [
        Column(
          ...
          children: [
            ...
            _doubleField('Image Aspect Ratio (W/H)', imageAspectRatio,
                (v) => imageAspectRatio = v),
            const SizedBox(height: 6),
            _doubleField('Active Fill Opacity', activeFillOpacity,
                (v) => activeFillOpacity = v.clamp(0.0, 1.0)),
            _doubleField('Inactive Fill Opacity', inactiveFillOpacity,
                (v) => inactiveFillOpacity = v.clamp(0.0, 1.0)),
            _doubleField('Border Width', borderWidth,
                (v) => borderWidth = v.clamp(0.0, 20.0)),
            _doubleField('Border Radius', borderRadius,
...
class ChargedUpScoringLevelWidget extends NTWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Listen to BOTH model changes (edit UI) and NT value changes
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        return Center(
          child: AspectRatio(
            aspectRatio: model.imageAspectRatio <= 0 ? (612 / 408) : model.imageAspectRatio,
            child: Stack(
              fit: StackFit.expand,
              children: [
                Image.asset(
  ...
  // Same placement logic, but NOT blinking anymore.
  Widget _levelRect({
    ...
    final showText = model.showLabels; // only selected is shown anyway
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.green.withOpacity(fillOpacity.clamp(0.0, 1.0)),
